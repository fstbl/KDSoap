...
static void showHelp(const char *appName)
{
    ...
    fprintf(stderr, "Usage: %s [options] [-impl <headerfile>] <wsdlfile>\n\n"
            ...
            "  -ignore-ssl-errors        ignore ssl errors when using a https url in the wsdlfile"
            "\n", appName);
}
...
int main( int argc, char **argv )
{
    ...
    while (arg < argc) {
        ...
        if (opt == QLatin1String("-h") || opt == QLatin1String("-help")) {
            ...
            nameSpace = argv[arg];
        } else if ( opt == QLatin1String("-ignore-ssl-errors") ) {
    ...
    Settings::self()->setIgnoreSslErrors(ignoreSslErrors);
